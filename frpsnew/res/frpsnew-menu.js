function openssHint(itemNum){
    ...
    if(itemNum == 0){
        ...
        statusmenu ="此处填入你的frp服务器的服务端口，对应服务器配置文件中的bindPort字段";
        _caption = "bindPort";
    }
    else if(itemNum == 2){
        statusmenu ="此处填入你的frp服务器的特权授权码。对应服务器配置文件中的auth.token字段。<br/><font color='#F46'>注意：</font>使用带有特殊字符的密码，可能会导致frpc链接不上服务器。";
        _caption = "auth.token";
    }
    else if(itemNum == 3){
