...
<script type="text/javascript" src="/res/frpsnew-menu.js"></script>
...
function get_dbus_data() {
	$.ajax({
		...
		success: function(data) {
			db_frpsnew = data.result[0];
			console.log(db_frpsnew);
		}
	});
}
function get_status(){
	var id = parseInt(Math.random() * 100000000);
	var postData = {"id": id, "method": "frpsnew_status.sh", "params":[1], "fields": ""};
	$.ajax({
		type: "POST",
		cache:false,
		url: "/_api/",
...
function conf2obj(){
	for (var i = 0; i < params_input.length; i++) {
		if(db_frpsnew[params_input[i]]){
			E(params_input[i]).value = db_frpsnew[params_input[i]];
		}
	}
	for (var i = 0; i < params_check.length; i++) {
		if(db_frpsnew[params_check[i]]){
			E(params_check[i]).checked = db_frpsnew[params_check[i]] == 1 ? true : false
		}
	}
}
function save() {
	...
	for (var i = 0; i < params_input.length; i++) {
		if (E(params_input[i]).value) {
			db_frpsnew[params_input[i]] = E(params_input[i]).value;
		}else{
			db_frpsnew[params_input[i]] = "";
		}
	}
	for (var i = 0; i < params_check.length; i++) {
		db_frpsnew[params_check[i]] = E(params_check[i]).checked ? '1' : '0';
	}
	var uid = parseInt(Math.random() * 100000000);
	console.log("uid :", uid);
	var postData = {"id": uid, "method": "frpsnew_config.sh", "params": ["web_submit"], "fields": db_frpsnew };
	$.ajax({
		url: "/_api/",
		cache: false,
		type: "POST",
...
function menu_hook(title, tab) {
	...
	tablink[tablink.length - 1] = new Array("", "Module_frpsnew.asp");
}
	...
	$.ajax({
		...
		success: function(res) {
			if(typeof(res["version"]) != "undefined" && res["version"].length > 0) {
				if(res["version"] == db_frpsnew["frpsnew_version"]){
					$("#frpsnew_version_show").html(" - " + res["version"]);
				}else if(res["version"] > db_frpsnew["frpsnew_version"]) {
					$("#frpsnew_version_show").html("<font color=\"#66FF66\">【有新版本：" + res.version + "】</font>");
				}
			}
		}
	});
...
function showWBLoadingBar(){
	...
	E("loading_block_title").innerHTML = "【frpsnew】日志";
	var page_h = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
	var page_w = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
	var log_h = E("loadingBarBlock").clientHeight;
	var log_w = E("loadingBarBlock").clientWidth;
							...
							<table width="760px" border="0" cellpadding="5" cellspacing="0" bordercolor="#6b8fa3" class="FormTitle" id="FormTitle">
								<tr>
									<td bgcolor="#4D595D" colspan="3" valign="top">
										<div>&nbsp;</div>
										<div class="formfonttitle">Frps内网穿透<lable id="frpsnew_version_show"><lable></div>
										...
										<div id="frpsnew_main">
												...
												<tr id="switch_tr">
													...
													<td colspan="2">
														<div class="switch_field" style="display:table-cell;float: left;">
															<label for="frpsnew_enable">
																<input id="frpsnew_enable" class="switch" type="checkbox" style="display: none;">
											...
											<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable" style="margin-top:8px;">
												...
												<td>
													<div id="frpsnew_status"><i><span id="status">获取中...</span></i></div>
												</td>
												<tr>
													<th width="20%"><a class="hintstyle" href="javascript:void(0);" onclick="openssHint(1)">bindPort</a></th>
													<td>
												<input type="text" class="input_ss_table" id="frpsnew_bindPort" name="frpsnew_bindPort" maxlength="5" value="" />
													</td>
												</tr>
												<tr>
													<th width="20%"><a class="hintstyle" href="javascript:void(0);" onclick="openssHint(2)">auth.token</a></th>
													<td>
														<input type="password" name="frpsnew_auth_token" id="frpsnew_auth_token" class="input_ss_table" autocomplete="new-password" autocorrect="off" autocapitalize="off" maxlength="256" value="" onBlur="switchType(this, false);" onFocus="switchType(this, true);"/>
													</td>
												</tr>
												<tr>
												...
												<tr>
													...
													<td>
														...
														<select id="frpsnew_common_cron_hour_min" name="frpsnew_common_cron_hour_min" style="width:60px;vertical-align: middle;" class="input_option">
